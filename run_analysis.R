...
all_data[,563] <- sapply(all_data[,563], function(x) gsub(1, "WALKING", x))
all_data[,563] <- sapply(all_data[,563], function(x) gsub(2, "WALKING_UPSTAIRS", x))
all_data[,563] <- sapply(all_data[,563], function(x) gsub(3, "WALKING_DOWNSTAIRS", x))
all_data[,563] <- sapply(all_data[,563], function(x) gsub(4, "SITTING", x))
all_data[,563] <- sapply(all_data[,563], function(x) gsub(5, "STANDING", x))
all_data[,563] <- sapply(all_data[,563], function(x) gsub(6, "LAYING", x))

# 2. Extract only the measurements on the mean and standard deviation for each measurement. 
filters <- grep("mean|std|Subject|Activity", names(all_data))
all_data_filtered <- all_data[,c(filters)]

# 5. From the data set in step 4, create a second, independent tidy data set 
# with the average of each variable for each activity and each subject.
#tidy_data<-aggregate(all_data_filtered[,1:79], by=list(all_data_filtered$Subject,all_data_filtered$Activity),FUN=mean)
data_melted <- melt(all_data_filtered, id.var = c("Subject", "Activity"))
